...
// Page identity color for Video Solutions — blue, distinct from the red
// used on the pure-delete Admin page.
const ACCENT = "#3d7fc0";
...
function AdminVideo() {
    ...
    const handleDelete = async (id) => {
        setDeletingId(id);
        try {
            await axiosClient.delete(`vid/delete/${id}`);
            
            setNotice({ type: 'success', text: 'Problem video deleted successfully.' });
        } catch (err) {
            console.error('error deleting problem', err);
            setNotice({ type: 'error', text: err.response?.data?.message || 'Could not delete the video of the problem' });
        } finally {
            setDeletingId(null);
            ...
            <div className="relative z-10 max-w-3xl mx-auto">
                ...
                <div className="flex flex-col items-center text-center mb-10">
                    <div
                        ...
                        style={{ borderColor: ACCENT, boxShadow: `0 0 25px -5px ${ACCENT}99` }}
                    >
                        <span className="text-2xl font-bold" style={{ color: ACCENT }}>悟</span>
                    </div>
                    <h1 className="font-serif tracking-[0.2em] text-2xl sm:text-3xl text-[#e9dfc7] uppercase">
                        Video Solution For Trails
                    </h1>
                    <div className="flex items-center gap-2 mt-3">
                        <span className="h-px w-8" style={{ backgroundColor: `${ACCENT}80` }} />
                        <p className="text-[11px] tracking-[0.25em] uppercase" style={{ color: `${ACCENT}cc` }}>
                            Support Learner
                        </p>
                        <span className="h-px w-8" style={{ backgroundColor: `${ACCENT}80` }} />
                    </div>
                ...
                <div
                    className="bg-[#141210]/95 border shadow-[0_0_60px_-10px_rgba(61,127,192,0.25)]"
                    style={{ borderColor: `${ACCENT}4d` }}
                >
                    {loading && (
                        <div className="flex justify-center py-14">
                            <span className="loading loading-spinner loading-lg" style={{ color: ACCENT }}></span>
                        </div>
                    )}
                    ...
                    {!loading && !error && problems.map((prob, idx) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                <NavLink
                                    to={`/admin/videoupload/${prob._id}`}
                                    state={{ problemId: prob._id, title: prob.title }}
                                ...
                                {confirmingId === prob._id ? (
                                    ...
                                    <button
                                        ...
                                        className="text-[10px] tracking-[0.15em] uppercase px-3 py-1.5 border border-[#c0453d]/50 text-[#c0453d] hover:bg-[#c0453d]/10 transition-colors"
                                    >
                                        Delete
                                    </button>
                                )}
                ...
                <div className="flex justify-center mt-8">
                    <NavLink
                        to="/admin"
                        className="text-[11px] tracking-[0.15em] uppercase hover:opacity-80 transition-opacity"
                        style={{ color: ACCENT }}
                    >
